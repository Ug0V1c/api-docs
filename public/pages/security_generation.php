    ...
    <main class="flex-1 min-w-0 lg:ml-80">
        <div class="max-w-4xl mx-auto p-6 lg:p-8">
            ...
            <div class="flex items-center gap-2 text-sm text-muted-foreground mb-8">
                ...
                <span class="text-foreground font-medium">Security: Generation Snippets</span>
            </div>
...
<span class="text-blue-400">public function</span> <span class="text-amber-200">getChecksumAirtimeData</span>(){
    ...
    $concatString = $loginId .<span class="text-green-400">"|"</span>. $requestId.<span class="text-green-400">"|"</span>. $serviceId.<span class="text-green-400">"|"</span>.$requestAmount.<span class="text-green-400">"|"</span>.$privateKey.<span class="text-green-400">"|"</span>. $recipient;

    $checksum = <span class="text-amber-200">base64_encode(password_hash</span>($concatString, <span class="text-red-500">PASSWORD_DEFAULT</span>) ); //PASSWORD_BCRYPT
...
<span class="text-blue-400">static</span> <span class="text-amber-200">String getChecksumAirtimeData</span>(){
    ...
    String concatString = <span class="text-green-400">""</span>+loginId +<span class="text-green-400">"|"</span>+requestId+<span class="text-green-400">"|"</span>+serviceId+<span class="text-green-400">"|"</span>+requestAmount+<span class="text-green-400">"|"</span>+privateKey+<span class="text-green-400">"|"</span>+recipient+<span class="text-green-400">""</span>;
    byte[] message = Bcrypt.<span class="text-amber-200">hashpw</span>(concatString,Bcrypt.<span class="text-amber-200">gensalt</span>()).<span class="text-amber-200">getBytes</span>(StandardCharsets.<span class="text-red-500">UTF_8</span>);
...
<span class="text-blue-400">public</span> string <span class="text-amber-200">getCheckSum</span>() {
    ...
    <span class="text-blue-400">var</span> concatText = loginId + <span class="text-green-400">"|"</span> + requestId + <span class="text-green-400">"|"</span> + serviceId + <span class="text-green-400">"|"</span> + amount + <span class="text-green-400">"|"</span> + privatekey + <span class="text-green-400">"|"</span> + recipient;
    ...
    <span class="text-blue-400">return</span> checkSum = Convert.<span class="text-amber-200">ToBase64String</span>(Encoding.UTF8.<span class="text-amber-200">GetBytes</span>(hashedText));
}
                            </pre>
                        </div>
                    </div>
